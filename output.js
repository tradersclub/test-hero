...
const Form = () => {
  ...
  return (
    ...
    test-id="3fb98c00c2">
      ...
      test-id="bc3c62346b"/>
      <Box display="flex" alignItems="center" flexDirection="column" overflow="auto" test-id="c7af4e8855">
        <Box paddingX={16} test-id="f025a1176d">
          <Box
          ...
          test-id="5dacc8d44c">
            {currentUser?.user && <IdeasAvatar user={currentUser?.user} size="md" test-id="c288eede03"/>}
            {isSimplePublication ? (
              <FreeIdea idea={idea} onCreate={createIdea} onUpdate={updateIdea} test-id="03e6b33985"/>
            ) : (
              <TradeIdea idea={idea} onCreate={createIdea} onUpdate={updateIdea} test-id="f921f4709f"/>
            )}
          </Box>
        </Box>
      </Box>
      <ModalExitForm isOpen={openModalExitForm} onClose={() => setOpenModalExitForm(false)} test-id="f6925218aa"/>
    </Box>
  );
};
